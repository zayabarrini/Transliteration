#!/usr/bin/env python3
"""
epub2jsonFrench.py - Convert French-original multilingual EPUB to JSON

This script is specifically designed for EPUBs where:
- French (fr) is the original without lang attribute
- Other languages have explicit lang attributes (en, ar, hi, ja, ko, zh, ru)
- Maintains position tracking for proper grouping across file boundaries
"""
...
# Order observed: en, ar, hi, ja, ko, zh, fr, ru
DEFAULT_LANGUAGE_ORDER = ['fr', 'pt', 'de', 'en', 'ru', 'hi', 'ar',  'ko', 'ja', 'zh']
...
def extract_content_with_position(file_path, start_position=0):
    """
    Extract all p tags (and h2 if present) in order.
    French is identified as the text without lang attribute.
    ...
    try:
        ...
        # Find all p tags (and include h2 if present for titles)
        all_elements = soup.find_all(['p', 'h2'])
        ...
        for elem in all_elements:
            ...
            # Skip if text is just whitespace or very short markers
            if len(text) < 2:
                continue
            ...
            # For h2 tags, they might be titles
            if elem.name == 'h2':
                if elem_lang:
                    lang = elem_lang
                else:
                    # French original for h2 if no lang attribute
                    lang = 'fr'
            else:
                # For p tags
                if elem_lang:
                    # Has explicit language attribute
                    lang = elem_lang
                else:
                    # No language attribute - this is French (original)
                    lang = 'fr'
...
def extract_all_content_with_positions(text_files):
    """
    ...
    for xhtml_file in text_files:
        ...
        content_items, current_position = extract_content_with_position(xhtml_file, current_position)
...
def detect_language_order_from_content(content_items):
    ...
    # Ensure fr is included and at the correct position
    if 'fr' not in order:
        order.append('fr')
...
def extract_title_from_epub(text_files):
    ...
    for xhtml_file in text_files:
        try:
            ...
            # Look for h1 or h2 with French text (no lang attribute)
    ...
    return "French Multilingual Text"

def process_french_epub(epub_file, language_order=None, output_file=None, output_base=None):
    """
    Process the French-original EPUB file.
    """
    ...
    print("French (fr) detected as original text without lang attribute")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print("\nExtracting content with position tracking...")
        all_content = extract_all_content_with_positions(text_files)
        ...
        if output_file:
            ...
            json_filename = f"{epub_name}-french-original.json"
...
def main():
    parser = argparse.ArgumentParser(description='Convert French-original multilingual EPUB to JSON')
    ...
    parser.add_argument('--lang-order', help='Comma-separated list of language codes in order (e.g., "en,ar,hi,ja,ko,zh,fr,ru")')
    ...
    print(f"\n📚 Processing French-original multilingual EPUB: {args.epub_file}")
    print(f"📁 Output base directory: {args.output_base}")
    
    sections = process_french_epub(
        args.epub_file,
        language_order=language_order,
    ...
    if sections:
        ...
        print(f"  🇫🇷 Original language: {sections[0]['original_language']}")
        ...
        if args.output:
            ...
            print(f"\n  💾 Output: {args.output_base}/{output_name}-french-original.json")
