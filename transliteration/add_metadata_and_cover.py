...
def add_metadata_and_cover(epub_folder, base_name, language, date=None):
    ...
    if dc_creator:
        ...
        dc_creator = opf_soup.new_tag('dc:creator', attrs={"opf:role": "aut", "opf:file-as": "Barrini, Zaya"})
        dc_creator.string = metadata["author"]
    ...
    source_image_path = f"/home/zaya/Downloads/Zayas/zayaweb/static/css/img/Bing/bing{random_number}.png"
    media_folder = os.path.join(epub_folder, "EPUB", "media") if os.path.exists(os.path.join(epub_folder, "EPUB")) else os.path.join(epub_folder, "OEBPS", "media")
    ...
    if cover_item:
        ...
        cover_item = opf_soup.new_tag('item', attrs={"id": "cover", "href": os.path.relpath(dest_image_path, os.path.dirname(opf_path)), "media-type": "image/png"})
