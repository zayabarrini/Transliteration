...
CONFIG_PRESETS = {
    'keep_russian_after_english': {
        'option': 3,
        'language_to_keep': 'ru',
        'language_after': 'en'
    },
    'keep_english_after_chinese': {
        'option': 3, 
        'language_to_keep': 'en',
        'language_after': 'zh'
    },
...
def _keep_only_language_after_no_lang(root, language_to_keep: str):
    ...
    for ref_para in reference_paragraphs:
        ...
        while current_elem is not None and not found:
            ...
            if (etree.QName(current_elem).localname == 'p' and 
                current_elem.get('lang') == language_to_keep):
                paragraphs_to_keep.add(current_elem)
                found = True
                break
            current_elem = current_elem.getnext()
    
    print(f"Keeping {len(paragraphs_to_keep)} {language_to_keep} paragraphs (after no-lang originals)")
...
def _keep_only_language_after_another(root, language_to_keep: str, language_after: str):
    ...
    print(f"Keeping {len(paragraphs_to_keep)} {language_to_keep} paragraphs (after {language_after})")
...
def process_epub_with_preset(epub_path: str, preset_name: str = 'keep_russian_after_english') -> str:
