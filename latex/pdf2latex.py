...
class PDFToLaTeXConverter:
    ...
    def _extract_pdf_content(
        self, pdf_path: Path, extract_equations: bool, preserve_layout: bool
    ...
    def _mathpix_recognize(self, image: Image.Image) -> Optional[str]:
        ...
        try:
            ...
            data = {"src": f"data:image/png;base64,{img_str}", "formats": ["latex_styled"]}

            response = requests.post(
                "https://api.mathpix.com/v3/text", headers=headers, data=json.dumps(data)
            )
    ...
    def _extract_images(self, pdf_path: Path) -> List[Dict]:
        ...
        for page_num in range(len(doc)):
            ...
            for img_index, img in enumerate(image_list):
                images.append({"page": page_num + 1, "image_index": img_index, "xref": img[0]})
    ...
    def _generate_latex_document(
        self, text_content: str, equations: List[Dict], images: List[Dict], metadata: Dict
    ) -> str:
...
def main():
    ...
    parser.add_argument("--no-equations", action="store_true", help="Skip equation extraction")
    parser.add_argument("--simple-layout", action="store_true", help="Use simple layout extraction")
    ...
    if args.mathpix_id and args.mathpix_key:
        mathpix_credentials = {"app_id": args.mathpix_id, "app_key": args.mathpix_key}

    # Create converter and process PDF
    converter = PDFToLaTeXConverter(
        use_mathpix=bool(mathpix_credentials), mathpix_credentials=mathpix_credentials
    )
